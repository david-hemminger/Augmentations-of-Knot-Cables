...
\begin{align*}
\left(\sum_{l=1}^k \left(\Phi_\alpha^L\right)_{q_i+1,l}a_{l*}\right)\otimes a_{r_i*} &= \left(\phi^\ast_{\alpha}\otimes \id\right)\psi^\ast\left(a_{i*}\right)\\
...
The arguments then proceed analogously to the first, with one qualification. In the third case, with $j\in X_{n+1}^{(p)}$, we must account for the condition $y_1\ne j-p$ in each summand. This causes the part of the sum not sent by $\psi$ to zero to vary slightly, depending on whether $i$ is larger, equal, or smaller than $j-p$. The expression in the statement of the lemma incorporates the three situations.
\end{proof}
...
Furthermore, for $\beta$ any braid, $\phi_\beta$ and $\psi$ both commute with conjugation, so we only need prove that 
      ...
      \end{equation}
for $i<j$, possibly $j=\ast$. Given Lemma \ref{Sigma_n} check the equality in each case. 

In the first two cases both sides of (\ref{CommuteEqn}) equal $1\otimes a_{r_ir_j}$. 

When $j>(n+1)p, i\in X_{n+1}^{(p)}$, we may have $j=\ast$. Since $q_i+1=n+1$, we have $\psi^\ast(a_{i-p,j})=a_{q_i,\ast}\otimes a_{r_i,\ast}=(\phi^\ast_{\s_n}\otimes\id)\psi^\ast(a_{i,\ast})$. If $j\le kp$ then $\psi(a_{i-p,j}) = a_{q_i,q_j+1}\otimes x$ where $x=a_{r_ir_j}, 1$ or $0$ depending on the relation of $r_i$ to $r_j$. Again $q_i=n$, and $q_j+1>n+1$, so $a_{q_i,q_j+1}=\phi_{\s_n}(a_{q_i+1,q_j+1})$, proving the statement. The case $i\le (n-1)p,j\in X_{n+1}^{(p)}$ is similar.
...
%\begin{rem}
%The fact that $\psi(a_{i,\a_i}a_{\a_i,y} - a_{iy}) = 0$ and $\psi$ behaves similarly for the analogous terms in cases 5 and 7 is the key to this proof working, and $\psi$ is defined the way it is mainly so that this will be true.  As we hinted at earlier, the homomorphism $\rho\colon \A_{kp}\rightarrow \A_k$ defined to send $a_{ij}$ to $a_{q_i+1,q_j+1}$ if $r_i=r_j$ and to 0 otherwise would also send these terms to 0, so Proposition \ref{psiofbp} would still be true with $\rho$ used in the place of $\psi$.  We will need $\psi$ for the proof of the main result, however.
%\end{rem}
...
\noindent Then, as $q_{i+p} = n<q_j$ we get (replace $q_j+1$ with $\ast$ if $j=\ast$)
$$\psi^\ast(a_{i+p,j} - a_{i+p,i}a_{i,j}) = (a_{n+1,q_j + 1} - a_{n+1,n}a_{n,q_j + 1})\otimes x = (\phi^\ast_{\s_n} \otimes \id)(\psi^\ast(a_{ij})),$$
\noindent with $x$ either as before, or $x=a_{r_i,\ast}$ if $j=\ast$.

Finally, suppose $i\in X_n^{(p)},j\in X_{n+1}^{(p)}$. Then $q_j=q_i+1=n$ and $\a_i=i$. If $j-p<i$ then $r_j<r_i$. By Lemmas \ref{Sigma_n} and \ref{SimplifiedImages}
\begin{align*}
...
We turn to the proof of Lemma \ref{Sigma_n}. Define $\t_{m,l} = \s_m\s_{m+1}\cdots\s_{m+l-1}$ and let $\kappa_{m,l} = \t_{m+l-1,p}\t_{m+l-2,p}\cdots\t_{m,p}$.  We show that if $i<j$ and $l\le p$ then
