...
To be precise, denote the group of braids on $n$ strands by $B_n$ and write $\hat{B}$ for the \emph{braid closure} of $B$ (see Section \ref{SecBG}, Figure \ref{fig:BClosure}). Consider $B\in B_k$ and $B''\in B_p$ and set $K = \hat{B}$. Note that $\ar(K)\le k$.
%Throughout the paper only braid closures that are knots are considered.

\begin{defn} Let $\fp B$ be the braid in $B_{kp}$ obtained by replacing each strand of $B$ by $p$ parallel copies (in the blackboard framing). Let $B'$ be the inclusion of $B''$ into $B_{kp}$ by the map $\s_i\mapsto\s_i, 1\le i\le p-1$. The \emph{braid satellite} of $K$ associated to $B, B''$ is defined as $K(B,B'') = \hat{\fp B\cdot B'}$.
\label{defn:BraidSat}
\end{defn}
\todo[color=\ltgreen]{make a figure} 
\todo[color=\ltgreen]{remark on difference from $B'(K)$ in satellite literature}

As defined $K(B,B'')$ depends on the choice of braid $B$ that represents the companion knot $K$. To make it a more intrinsic satellite construction we can require that the index $k$ of $B\in B_k$ is minimal among braid closures representing $K$ (see Section \ref{SecAsSatelliteOp} for a proof of this fact). 

Note that if $\hat{B}$ and $\hat{B''}$ are each a knot, $K(B,B'')$ is also. Our principal result is the following.

\begin{thm}\label{main}
If $B\in B_k$ and $B''\in B_p$ are such that $\ar(\hat{B})=k$ and $\ar(\hat{B''})=p$, then $\ar(K(B,B''))=kp$.
\end{thm}
...
The knot $T({\bf p},{\bf q})$ is well-defined when a framing convention is chosen at each stage of cabling. In contrast to the traditional choice of Seifert framing at each stage, we choose a framing so that if $B$ is of minimal braid index such that $T(\hat{\bf p},\hat{\bf q}) = \hat{B}$ then $T({\bf p},{\bf q}) = K(B,(\s_1\ldots\s_{p_n-1})^{q_n})$.
...
It is natural to wonder if the augmentation rank is multiplicative under assumptions on $B, B''$ weaker than those in Theorem \ref{main}. The following is a possible generalization.

\begin{conj}Suppose $K=\hat{B}$ for $B\in B_k$, and that $B$ has minimal index among braids with the same closure. Let $B''\in B_p$. Then $\ar(K(B,B'')) \ge \ar(\hat{B})\ar(\hat{B''})$.
...
Definition \ref{defn:BraidSat} of the braid satellite $K(B,B'')$ produces a satellite of $\hat{B}$. As defined, the resulting satellite depends on the chosen braid representative of $\hat{B}$. We remark here how to avoid this ambiguity.
...
\begin{prop}\label{PropAsSatelliteOp}Given a knot $K$ and a braid $B''\in B_p$, let $\omega$ be the writhe of a minimal index closed braid representing $K$. Let $P\subset S^1\times D^2$ be the braid closure of $\Delta^{2\omega}B''$, where $\Delta^2$ is the full twist in $B_p$. Then $K(B,B'') = P(K)$ for any minimal index braid $B$ with $K=\hat{B}$.
\end{prop}
\begin{proof}The principal observation is that, since the Jones conjecture holds \cite{DP12,LM12}, the writhe of $B$ must be $\omega$. Thus the blackboard framing of the closure of $\fp B\Delta^{-2\omega}$ agrees with the $(p,0)$-cable of $K$ (with Seifert framing).
\end{proof}
...
\label{SecBG}
  ...
  Throughout the paper we denote by $B_n$ the group of $n$-braids. We orient $n$-braids from left to right, labeling the strands $1,\ldots, n$, with 1 the topmost and $n$ the bottommost strand. We work with Artin's generators $\{\sigma_i^{\pm}$, $i=1,\ldots,n-1\}$ of $B_n$, where in $\s_i$ only the $i$ and $i+1$ strands interact, and they cross once in the manner depicted in Figure \ref{fig:BraidGens}.
      \begin{figure}[ht]
        ...
        \label{fig:BraidGens}
      \end{figure}
    Given a braid $B$ the braid closure $\hat{B}$ of $B$ is the link obtained as shown in Figure \ref{fig:BClosure}. The \emph{writhe} (or algebraic length) of $B$, denoted $\omega(B)$, is the sum of exponents of the Artin generators in a word representing $B$.
...
\begin{main}
Let $B\in B_k$ have augmentation rank $k$, and let $B''\in B_p$ have augmentation rank $p$.  If $B'$ is the braid $B''$ included into $B_{pk}$, then $\fp BB'$ has augmentation rank $pk$.
\end{main}

Theorem \ref{main} is proved by defining an algebra map $\psi\colon \A_{pk} \rightarrow \A_k\otimes \A_p$ such that $\psi(\Phi_{\fp BB'}^L)$ factors suitably across the tensor product. This is the content of Proposition \ref{psiofbp}, which follows from Lemmas \ref{commutes} and \ref{basecase}, the former following from a calculation (Lemma \ref{Sigma_n}) and the latter depending on the former. %We begin with the definition of $\psi$ and statement of Proposition \ref{psiofbp}, followed by the proof of Proposition \ref{psiofbp} and Lemmas \ref{commutes}, \ref{basecase}, and \ref{Sigma_n}.
...
For any braid $B$, $\psi\left(\Phi_{\fp B}^L\right) = \left(\left(\Phi_B^L\right)_{ij}\otimes 1\right)\otimes I_p$ and $\psi\left(\Phi_{\fp B}^R\right) = \left(\left(\Phi_B^R\right)_{ij}\otimes 1\right)\otimes I_p$\todo[color=\ltblue]{make consistent throughout paper}
\end{prop}


Note that here we mean the tensor product of $\Phi_B^L$ and $I_p$ as matrices, not as linear maps, while the tensor product of $\left(\Phi_B^L\otimes I_p\right)_{ij}$ and $1$ is a tensor product of algebra elements, so that if we divide the matrix $\psi\left(\Phi_{\fp B}^L\right)$ into $k^2$ $p\times p$\todo{awkward} blocks, the $ij$th block is $\left(\Phi_B^L\right)_{ij}I_p$.
...
\begin{equation}
\pi\circ(\epsilon_k\otimes\delta)\circ\psi\left(\Phi_{\fp BB'}^L\right) = \pi\circ(\epsilon_k\otimes\delta)\psi\left(\phi_{\fp B}\left(\Phi_{B'}^L\right)\right)\psi\left(\Phi_{\fp B}^L\right)
\label{eqn1MainPf}
\end{equation}

Consider the homomorphism $\phi_{\fp B}$ through the description given in Section \ref{SecBG_KCHdef}. As each $a_{ij}, 1\le i\ne j\le p$, is represented by the isotopy class $c_{ij}$, and the leftmost $p$ punctures are moved as one block by the action of $\fp B$ on $D$, there is an $0\le m<k$ so that $\phi_{\fp B}(a_{ij})=a_{i+mp,j+mp}$ for each $i,j$ in this range. As $\psi(a_{i + mp, j+mp})=1\otimes a_{ij}$,

$$\psi\left(\phi_{\fp B}\left(\Phi_{B'}^L\right)\right) = \left(1\otimes \left(\Phi_{B'}^L\right)_{ij}\right)$$

\noindent By Proposition \ref{psiofbp}, we have that 

$$\psi\left(\Phi_{\fp B}^L\right) = \left(\left(\Phi_B^L\right)_{ij}\otimes 1\right)\otimes I_p = \left(\left(\Phi_B^L\otimes I_p\right)_{ij}\otimes 1\right)$$\todo{this is maybe confusing, since $i,j$ in the middle part range over different values than $i,j$ on the RHS}

\noindent Returning to the right hand side of (\ref{eqn1MainPf})
\begin{align*}
\pi\circ(\epsilon_k\otimes\delta)\left(\psi\left(\phi_{\fp B}\left(\Phi_{B'}^L\right)\right)\psi\left(\Phi_{\fp B}^L\right)\right)
...
We are done if $\delta$ may be defined so that $\delta\left(\Phi_{B'}^L\right)\left(\Delta(B)\otimes I_p\right) = \Delta(\fp BB')$.  When $w(B)$ is even $w(\fp B)$ is also, and further $\Delta(B)=I_k$. Letting $\delta = \epsilon_p$ makes 
\begin{equation*}
\delta\left(\Phi_{B'}^L\right)\left(\Delta(B)\otimes I_p\right) = \epsilon_p\left(\Phi_{B'}^L\right) = \Delta(B') = \Delta(\fp BB').
\end{equation*}
...
When $p$ is even, $w(\fp B)$ is also even and so the opposite parity of $w(B)$. Our definition of $g$ gives $\delta\left(\left(\Phi_{B'}^L\right)_{ii}\right) = -\epsilon\left(\left(\Phi_{B'}^L\right)_{ii}\right)$ for $i\le p$. Thus 
...
$$
\delta\left(\Phi_{B'}^L\right)\left(\Delta(B)\otimes I_p\right) = \diag[(-1)^{w(B) + w(B') + 1},1\ldots 1] = \Delta(\fp BB')
$$
\noindent as desired.

When $p$ is odd, $w(\fp B)$ is odd and therefore the same parity of $w(B)$. Our definition of $g$ gives that $\delta\left(\left(\Phi_{B'}^L\right)_{11}\right) = \epsilon\left(\left(\Phi_{B'}^L\right)_{11}\right)$ and $\delta\left(\left(\Phi_{B'}^L\right)_{ii}\right) = -\epsilon\left(\left(\Phi_{B'}^L\right)_{ii}\right)$ for $1<i\le p$, so 
...
\noindent and therefore
$$
\delta\left(\Phi_{B'}^L\right)\left(\Delta(B)\otimes I_p\right)= \diag[(-1)^{w(B) + w(B')},1\ldots 1] = \Delta(\fp BB')
$$
\noindent as desired. 

There is little difference in the proof that $\epsilon(\Phi_{\fp B}^R) = \Delta(\fp BB')$, except that monomials in $(\Phi_{B'}^R)_{ij}$ are of the form $c_{ij}a_{i,j_1}a_{j_1,j_2}\cdots a_{j_k,j'}$ where $j'=\text{perm}(B')(j)$. Applying Theorem \ref{thm:RanknAugs} now completes the proof.
%Similarly, we have that
%$$
%\pi\circ(\epsilon_k\otimes\delta)\circ\psi\left(\Phi_{\fp BB'}^R\right) = \pi\circ(\epsilon_k\otimes\delta)\left(\left(\left(\Phi_B^R\otimes I_p\right)_{ij}\otimes 1\right)\left(1\otimes \left(\Phi_{B'}^R\right)_{ij}\right)\right)
%$$
%but since $\epsilon_k\left(\Phi_B^L\right) = \epsilon_k\left(\Phi_B^R\right)$ and $\epsilon_p\left(\Phi_{B'}^L\right) = \epsilon_p\left(\Phi_{B'}^R\right)$, in each of the above cases we have that the right hand side is equal to $\Delta\left(\fp BB'\right)$, which completes the proof.
\end{proof}
...
\begin{rem}As the map $B_{k} \to \text{Aut}(\A_k\otimes\A_p)$ given by $B \mapsto \phi_B\otimes\id$ is a homomorphism, Lemma \ref{commutes} immediately implies that $\psi(\phi_{\fp B}(a_{ij})) = (\phi_{B} \otimes \id)(\psi(a_{ij}))$ for any $B\in B_k$.
\end{rem}
...
Let $B = \s_{n_1}^{q_1}\cdots\s_{n_r}^{q_r}$, where $1\le n_i<k$ and $q_i = \pm 1$.  We will prove the proposition by induction on $r$.  The base case is already taken care of by Lemma \ref{basecase}.  Suppose that the proposition holds for braids of length $r-1$.  Let $B' =\s_{n_1}^{q_1}\cdots\s_{n_{r-1}}^{q_{r-1}}$\todo{should I pick something other than $B'$?} Then by the Chain Rule and Lemmas \ref{commutes} and \ref{basecase}, we have that
\begin{align*}
\psi\left(\Phi_{\fp B}^L\right) &= \psi\left(\phi_{\fp B'}\left(\Phi_{\fp\s^{q_r}_{n_r}}^L\right)\cdot\Phi_{\fp B'}^L\right)\\
&= \left(\phi_{B'}\otimes\id\right)\left(\psi\left(\Phi_{\fp\s^{q_r}_{n_r}}^L\right)\right)\cdot \left(\left(\Phi_{B'}^L\right)_{ij}\otimes 1\right)\otimes I_p\\
...
\end{align*}
We also have then that $\psi\left(\Phi_{\fp B}^R\right) = \left(\left(\Phi_{B}^R\right)_{ij}\otimes 1\right)\otimes I_p$ as well, since $\Phi_B^R = \overline{\Phi_B^L}^t$ and $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$.
\end{proof}
...
Furthermore, $\phi_B(\overline{a_{ij}}) = \overline{\phi_B(a_{ij})}$\todo{does this need justification?} and $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$, so it suffices to prove the lemma for $\fp \s_n$ in the case where $i<j$.
...
Let $\tau = \s_1\ldots\s_{n-1}\in B_n$ and set $\alpha = \tau^{n+1}$, which has the $(n,n+1)$ torus knot as its braid closure. Set $\beta = \pp[-3]2\alpha$, then $K=\hat{\beta\s_1}$.

Let $k\ge1$. Every entry of $\Phi_{\pp 2\tau^k}^L$ is a polynomial in the (non-commuting) variables ${\bf a} = (a_{12},a_{13},\ldots,a_{2n-1,2n})$. For each $1\le i,j\le n$ write $A_{i,j}^k({\bf a})$ for entry $\left(\Phi_{\tau^k}^L\right)_{2i-1,2j-1}$ and similarly write $B_{i,j}^k({\bf a}), C_{i,j}^k({\bf a}),$ and $D_{i,j}^k({\bf a})$ for the entries in the $(2i-1,2j), (2i,2j-1),$ and $(2i,2j)$ spots, respectively.
...
Since $\Phi_{\pp2\tau^{k+1}}^L = \phi_{\pp2\tau}(\Phi_{\pp2\tau^{k}}^L)\Phi_{\pp2\tau}^L$ we see that for $2\le j\le n$
  \al{
    %A_{i,j}^{k+1}({\bf a}) &= A_{i,j-1}^k(\phi_{\pp 2\tau}({\bf a})) \\
    B_{i,j}^{k+1}({\bf a})  &= B_{i,j-1}^k(\phi_{\pp 2\tau}({\bf a})) \\
    %C_{i,j}^{k+1}({\bf a}) &= C_{i,j-1}^k(\phi_{\pp 2\tau}({\bf a})) \\
    D_{i,j}^{k+1}({\bf a})  &= D_{i,j-1}^k(\phi_{\pp 2\tau}({\bf a})).
  }
Also, since $\Phi_{\pp2\tau^{k+1}}^L = \phi_{\pp2\tau^k}(\Phi_{\pp2\tau}^L)\Phi_{\pp2\tau^k}^L$ we have that
  \al{
...
Combining these two you get that for $j\ge2$, $X_{1,j}^{k+1}({\bf a}) = X_{1,j-1}^k(\phi_{\pp2\tau}({\bf a})) = X_{n,j-1}^{k+1}(\phi_{\pp2\tau}({\bf a}))$ for $X\in\{B,D\}$.
Also, for $1\le i\le n-1$,
  \begin{align}
    %A_{i,j}^{k+1}({\bf a}) &= A_{i+1,j}^k({\bf a})+A_{i,1}^1(\phi_{\pp2\tau^k}({\bf a}))A_{1,j}^k({\bf a})+B_{i,1}^1(\phi_{\pp2\tau^k}({\bf a}))C_{1,j}^k({\bf a}) \label{eqn1}; \\
    B_{i,j}^{k+1}({\bf a})  &= B_{i+1,j}^k({\bf a})+A_{i,1}^1(\phi_{\pp2\tau^k}({\bf a}))B_{1,j}^k({\bf a})+B_{i,1}^1(\phi_{\pp2\tau^k}({\bf a}))D_{1,j}^k({\bf a}) \label{eqn2}.
  \end{align}

Starting at (\ref{eqn2}) with $i\le n-1,$ and $k=n$ we get
  \al{
    B_{i,j}^{n+1}({\bf a})  
      &= B_{i+1,j}^n({\bf a}) + A_{i,1}^1(\phi_{\pp2\tau^n}({\bf a}))B_{1,j}^n({\bf a})+B_{i,1}^1(\phi_{\pp2\tau^n}({\bf a}))D_{1,j}^n({\bf a}) \\
      &= B_{i+1,1}^n({\bf a}) + A_{i,1}^1(\phi_{\pp2\tau^n}({\bf a}))B_{n,j}^{n+1}({\bf a})+B_{i,1}^1(\phi_{\pp2\tau^n}({\bf a}))D_{n,j}^{n+1}({\bf a}).
  }
  Since $\varphi(B_{n,j}^{n+1}({\bf a}))=0$ and $\varphi(D_{n,j}^{n+1}({\bf a}))=0$ for $1\le j\le n$ by assumption, this says that $B_{i,j}^{n+1}$ and $B_{i+1,j}^n$ have the same image under $\varphi$. Using the first set of equations, this means that $\varphi(B_{i,j}^{n+1}({\bf a}))=\varphi(B_{i+1,j+1}^{n+1}(\phi_{\pp2\tau}^{-1}({\bf a})))$.

  Hence we have that $\varphi(B_{1,1}^{n+1}({\bf a})) = \varphi(B_{n,n}^{n+1}(\phi_{\pp2\tau}^{-n+1}({\bf a}))) = \varphi(B_{1,1}^1({\bf a})) = \varphi(-a_{32})$. We will show that $\varphi(a_{32}) = 0$.
...
\begin{lem}Suppose $\varphi:\mathcal A_{2n}\to \mathbb C$ is a homomorphism with the property $\varphi((\Phi_{\beta}^L)_{2i-1})={\bf e}_{2i-1}$ and $\varphi((\Phi_{\beta}^L)_{2i})={\bf e}_{2i}$ for $2\le i\le n$. Then $\varphi(a_{32}) = 0$.
\label{LemZeroness}
\end{lem}
\begin{proof}We arrive at the result by considering a computation of the row $(\Phi_{\beta}^L)_3$, which is obtained from the equation $\phi_{\beta}^{\ast}(a_{3,2n+1}) = \sum_{j=1}^{2n}(\Phi_{\beta}^L)_{3,j}a_{j,2n+1}$. Described as an element in the algebra of paths (see Section \ref{SecBG_KCHdef}), the image $\phi_{\beta}^{\ast}(a_{3,2n+1})$ is given up to isotopy by the curve in Figure \ref{FigPhiLCalcPhi}.
    ...
    \begin{figure}[ht]
      \begin{tikzpicture}[scale=.75,>=stealth]
        ...
        \draw (5,-1.25) node[below] {{\small $\beta\cdot c_{3,2n+1}$}};
      \end{tikzpicture}
      \caption{Computing third row of $\Phi_{\beta}^L$}
  ...
  Denote the curve on the left in Figure \ref{FigSentToZero} by $x_i$. Denote by $y_{ij}$ the curve on the right in the same figure, and let $y_{ij}=1$ if $i=j$. Using the relation \ref{FigRelnPathAlg} to pull the curve in Figure \ref{FigPhiLCalcPhi} across the rightmost $2n-6$ punctures, we consider how to get a coefficient of $a_{j,2n+1}$. For each $j\ge 7$ we get a coefficient of $a_{j,2n+1}$ as below.
      ...
      \begin{equation}\langle\phi_{\beta}^{\ast}(a_{3,2n+1}), a_{j,2n+1}\rangle = \sum_{i=j}^{2n}x_{i}y_{i,j}
  ...
  As $x_{2n}$ is the coefficient of $a_{2n,2n+1}$, our assumption makes $\varphi(x_{2n}) = 0$. This in turn implies that $\varphi(x_{2n-1})=0$ by considering the case $j=2n-1$. Continuing in this way, we see that $\varphi(x_{j})=0$ for all $j>6$. Letting $w$ denote the curve of Figure \ref{FigPhiLCalcPhiReduced}, we see that (applying $\varphi$ to the coefficients of the $a_{j,2n+1}$) $\varphi(\phi_{\beta}^{\ast}(a_{3,2n+1})) = \varphi(w)$. We remark, the curve $w$ is the image of $c_{3,2n+1}$ under the braid $\phi_{\pp 2(\s_1\s_2)^4}$ in $B_6\subset B_{2n}$.
    ...
    \begin{figure}[ht]
      \begin{tikzpicture}[scale=.75,>=stealth]
        ...
        %           ..controls (5-1.1,0.69) and (5-1.45,0.99) ..(5-1.1,1.2)
        %           ..controls (5-0.4,1.62) and (5.7,2.2) ..(6.685,1.01);
        \draw (5,-1.25) node[below] {{\small $w = \pp 2(\s_1\s_2)^4\cdot c_{3,2n+1}$}};
      \end{tikzpicture}
  ...
  A similar argument to the above shows that the (coefficients of the) image under $\varphi$ of $\beta\cdot c_{4,2n+1}$, $\beta\cdot c_{5,2n+1}$, and $\beta\cdot c_{6,2n+1}$ also correspond to the image of $\pp 2(\s_1\s_2)^4\cdot c_{i,7}$. This implies that $\varphi$, when applied to the $6\times 6$ matrix $\Phi_{\pp 2(\s_1\s_2)^4}^L$, sends the last four rows to ${\bf e}_3,{\bf e}_4,{\bf e}_5,{\bf e}_6$. It can be directly calculated that any such map on $\mathcal A_6$ satisfies $\varphi(-a_{32}) = 0$, concluding the proof of the lemma.
  \end{proof}

To complete the theorem, recall that $K=\hat{\beta\s_1}$. Any augmentation with rank $2n$ would require a homomorphism $\epsilon: \mathcal A_{2n}\to\mathbb C$ such that $\epsilon\left(\Phi_{\beta\s_1}^L\right) = \Delta(\beta\s_1)$. By the Chain rule $\Phi_{\beta\s_1}^L = \phi_{\beta}(\Phi_{\s_1}^L)\Phi_{\beta}^L$ which, by the form of $\Phi_{\s_1}^L$, has (except for the first two rows) the same rows as $\Phi_{\beta}^L$.

As a consequence, $\epsilon$ must send the $i^{th}$ row of $\Phi_{\beta}^L$ to ${\bf e_i}$ for $i\ge 3$. By Lemma \ref{LemZeroness} we see that $\epsilon\left((\Phi_{\beta}^L)_{12}\right) = 0$. As $\Phi_{\beta\s_1}^L = \phi_{\beta}(\Phi_{\s_1}^L)\Phi_{\beta}^L$ indicates that $\Phi_{\beta\s_1}^L$ has a second row equal to the first row of $\Phi_{\beta}^L$, there is a diagonal entry in $\epsilon\left(\Phi_{\beta\s_1}^L\right)$ equal to zero, so it cannot equal $\Delta(\beta\s_1)$, a contradiction.
\end{proof}

\subsection{Augmentation rank does not multiply}
\label{SecMultiplic}

As discussed in Section \ref{SecAsSatelliteOp} the braid satellite $K(B,B'')$ depends only on $B''$ and the closure $\hat{B}$ if $B$ has minimal braid index. We give some motivation for Conjecture \ref{ConjSuperMultipl}, that $\ar(K(B,B''))\ge \ar(\hat{B})\ar(\hat{B''})$.

\begin{thm}For any braid $B$ with $K=\hat{B}$, we have $\ar(K(B,B'')) \ge \ar(K)$.
\label{ThmCompanionRank}
\end{thm}
\begin{proof}The knot group $\pi_{K(B,B'')}$ is isomorphic to a product of $\pi_K$ and the group for the closure of $\Delta^{2\omega}B''$ amalgamated along a $\mathbb Z^2$ coming from the torus boundary $T$ of the neighborhood $n(K)$. Choose the basepoint of $\pi_{K(B,B'')}$ on this torus and include $\pi_K$ into the amalgamated product via the inclusion of the complement.
...
Consider a collection of meridians $m_1,\ldots,m_r$ of $K$ that generate $\pi_K$. For each $1\le i\le r$ there are $p$ meridians $m_{i1},\ldots,m_{ip}$ of $K(B,B'')$ such that $m_{i1}m_{i2}\ldots m_{ip} = m_i$. Set $\s(m_{1j}) = \text{diag}[\mu_0,1,\ldots,1] = M$ for $1\le j\le p$. Then, for each $2\le i\le r$ find $w_i\in\pi_K$ so that $m_i = w_im_1w_i^{-1}$ and set $\s(m_{ij}) = \rho(w_i)M\rho(w_i)^{-1}$ for $1\le j\le p$.

As $\pi_{K(B,B'')}$ has a presentation with every relation of the form $m_{i,j} = w_im_{1,k}w_i^{-1}$ for some $1\le j, k\le p$ and $1\le i\le r$ we see that $\s:\pi_{K(B,B'')}\to\text{GL}_n\mathbb C$ is a well-defined KCH representation. Moreover, $\s$ has the same image as $\rho$, implying it is irreducible. This shows that $\ar(K(B,B''))\ge \ar(K)$.
\end{proof}

It should also be noted that, for $P=\hat{\Delta^{2\omega}B''}$, we have $\ar(K(B,B''))\ge \ar(P)$ also. This follows from Proposition \ref{PropAsSatelliteOp} and the fact that there is a surjection $\pi_{K(B,B'')}\to\pi_P$, preserving peripheral structures (see Proposition 3.4 in \cite{SW}, for example). 

Oddly, the product $\ar(K)\ar(P)$ does not relate well to $\ar(K(B,B''))$, even when $B$ has minimal index: from Theorem \ref{ThmNNPlus1} we find examples where $\ar(K(B,B''))<\ar(K)\ar(P)$ and from Theorem \ref{main} there are examples with $\ar(K(B,B''))>\ar(K)\ar(P)$. However, to our knowledge Conjecture \ref{ConjSuperMultipl} may hold as stated. 

There are cases where $\ar(K(B,B''))$ is strictly larger than $\ar(K)\ar(\hat{B''})$. One example can be found from the $(2,11)$-cable of the $(2,5)$ torus knot. By finding a solution to (\ref{eqn:FindingAugs}) for $B = {\scriptstyle \bar{\bar{2}}}\s_1^5\cdot \s_1\in B_4$, we can compute that $\ar(K(\s_1^5,\s_1)) = 4$, even though $\ar(\hat{\s_1^5})=2$ and $\ar(\hat{\s_1})=1$. Unfortunately, any more examples of cables of torus knots (not covered by Theorems \ref{main} and \ref{ThmNNPlus1}) seem outside of our computational abilities.

We end with a simple remark on computational observations. By the inequalities in (\ref{cor:DimBound}) if a knot has bridge number less than its minimal braid index $n$, it cannot have augmentation rank equal to $n$. Take a minimal index braid representative of such a knot, and multiply that braid by successively higher powers of $\Delta^2\in B_n$, testing in each instance if the closure has augmentation rank equal to $n$. In examples, the power of $\Delta^2$ need not be very high, compared to $n$, before a braid with augmentation rank $n$ appears. The existence of such an augmentation also seems to persist. Note that braids which are sufficiently far in Dehornoy's order from the identity were shown in \cite{MN} to not admit a Birman-Menasco template, and thus are minimal index representatives by the MTWS. For a given braid index $n$, is there a number $m_n$ such that whenever $\Delta^{2m_n}$ is less than $B\in B_n$ in the Dehornoy order, then $\ar(\hat{B}) = n$?
