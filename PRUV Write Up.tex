...
Theorem \ref{main} is proved by defining an algebra map $\psi\colon \A_{pk} \rightarrow \A_k\otimes \A_p$ such that $\psi(\Phi_{B^{(p)}B'}^L)$ factors suitably across the tensor product. This is the content of Proposition \ref{psiofbp}, which follows from Lemmas \ref{commutes} and \ref{basecase}, the former following from a calculation (Lemma \ref{Sigma_n}) and the latter depending on the former. %We begin with the definition of $\psi$ and statement of Proposition \ref{psiofbp}, followed by the proof of Proposition \ref{psiofbp} and Lemmas \ref{commutes}, \ref{basecase}, and \ref{Sigma_n}.
...
We are done if $\delta$ may be defined so that $\delta\left(\Phi_{B'}^L\right)\left(\Delta(B)\otimes I_p\right) = \Delta(B^{(p)}B')$.  When $w(B)$ is even $w(B^{(p)})$ is also, and further $\Delta(B)=I_k$. Letting $\delta = \epsilon_p$ makes 
\begin{equation*}
\delta\left(\Phi_{B'}^L\right)\left(\Delta(B)\otimes I_p\right) = \epsilon_p\left(\Phi_{B'}^L\right) = \Delta(B') = \Delta(B^{(p)}B').
\end{equation*}
...
When $p$ is odd, $w(B^{(p)})$ is odd and therefore the same parity of $w(B)$. Our definition of $g$ gives that $\delta\left(\left(\Phi_{B'}^L\right)_{11}\right) = \epsilon\left(\left(\Phi_{B'}^L\right)_{11}\right)$ and $\delta\left(\left(\Phi_{B'}^L\right)_{ii}\right) = -\epsilon\left(\left(\Phi_{B'}^L\right)_{ii}\right)$ for $1<i\le p$, so 
...
\begin{rem}As the map $B_{k} \to \text{Aut}(\A_k\otimes\A_p)$ given by $B \mapsto \phi_B\otimes\id$ is a homomorphism, Lemma \ref{commutes} immediately implies that $\psi(\phi_{B^{(p)}}(a_{ij})) = (\phi_{B} \otimes \id)(\psi(a_{ij}))$ for any $B\in B_k$.
\end{rem}
...
$$\psi\left(\phi_{\SpM_n}\left(a_{ij}\right)\right) = \left(\phi_{\s_n^{-1}}\otimes\id\right)\psi(a_{ij})$$
