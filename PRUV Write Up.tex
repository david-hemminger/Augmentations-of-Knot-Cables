...
Now suppose that $w(B)$ is odd.  In a moment we will define $g\colon \{1,\ldots,p\}\rightarrow \{\pm 1\}$ for each of the cases for when $p$ is even or odd, but for now let $\delta(a_{ij}) = g(i)g(j)e_k(a_{ij})$.  Fix $i,j$ and consider a monomial $M$ in $\left(\Phi_{B'}^L\right)_{ij}$.  Since $B'$ is a braid on $p$ strands included into $B_{pk}$, if $i>p$ or $j>p$ then $M$ is 0 or 1 and $\delta(M) = M$.  If $i,j\le p$, such a monomial must arise from a product in the algebra of paths in $D$ \todo[color=\ltblue]{just introducing this notation}\todo[color=\ltgreen]{need to make a note about roughly why this is true in background} that begins at $i' = \perm(B')(i)$ and ends at $j$, so $M=c_{ij}a_{i',j_1}a_{j_1,j_2}\ldots a_{j_m,j}$ for some $j_1,\ldots j_m\in \{1,\ldots,p\}$, unless $i' = j$, in which case it is possible that $M=c_{ij}$.  We then see that 
