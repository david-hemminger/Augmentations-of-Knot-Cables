...
\todo[inline, color=\ltblue]{introduce $q_i$ and $r_i$ notation}
...
Theorem \ref{main} is proved by defining an algebra map $\psi\colon \A_{pk} \rightarrow \A_k\otimes \A_p$ such that $\psi(\Phi_{\alpha\bar\gamma}^L)$ factors suitably across the tensor product. This is the content of Proposition \ref{psiofbp}, which follows from Lemmas \ref{commutes} and \ref{basecase}, the former following from a calculation (Lemma \ref{Sigma_n}) and the latter depending on the former. 
...
\noindent which determines an algebra map $\psi\colon \A_{pk} \rightarrow \A_k\otimes \A_p$. Note that if $\psi(a_{ij}) \in 1\otimes \A_p$ then $q_i = q_j$ or $a_{ij}\in\ker\psi$\todo[color=\ltblue]{is this used?  is converse used? don't include this here?}. Also, if we extend conjugation to $\A_k\otimes\A_p$ by applying it to each factor, then $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$. We have the following proposition.
...
Note that by $\Phi_\alpha^L\otimes I_p$ we mean the Kronecker product of $\Phi_\alpha^L$ and $I_p$, or equivalently their tensor product as $\A_k$- and $\A_p$-linear automorphisms of $\A_{k+1}$ and $\A_{p+1}$, respectively.\todo[color=\ltblue]{say more here}

%tensor product of the linear maps is a linear automorphism of A_k+1\otimes A_p+1 as a A_k\otimes A_p module
...
Consider the homomorphism $\phi_{\subpp\alpha}$ through the description \todo{specify this description better somehow} given in Section \ref{SecBG_KCHdef}. As each $a_{ij}, 1\le i\ne j\le p$, is represented up to sign by the isotopy class $c_{ij}$, and the leftmost $p$ punctures are moved as one block by the action of $\subpp\alpha$ on $D$, there is an $0\le m<k$ so that $\phi_{\subpp\alpha}(a_{ij})=a_{i+mp,j+mp}$ for each $i,j$ in this range. As $\psi(a_{i + mp, j+mp})=1\otimes a_{ij}$,

$$\psi\left(\phi_{\subpp\alpha}\left(\Phi_{\bar\gamma}^L\right)\right) = \left(1\otimes \left(\Phi_{\bar\gamma}^L\right)_{ij}\right)$$\todo{entries of $\Phi_{\bar\gamma}^L$ lie in $\A_{kp}$, but all of them are in image of inclusion of $\A_p$}
...
%$$
%\pi\circ(\epsilon_k\otimes\delta)\circ\psi\left(\Phi_{\fp B\bar\gamma}^R\right) = \pi\circ(\epsilon_k\otimes\delta)\left(\left(\left(\Phi_B^R\otimes I_p\right)_{ij}\otimes 1\right)\left(1\otimes \left(\Phi_{\bar\gamma}^R\right)_{ij}\right)\right)
%$$
%but since $\epsilon_k\left(\Phi_B^L\right) = \epsilon_k\left(\Phi_B^R\right)$ and $\epsilon_p\left(\Phi_{\bar\gamma}^L\right) = \epsilon_p\left(\Phi_{\bar\gamma}^R\right)$, in each of the above cases we have that the right hand side is equal to $\Delta\left(\fp B\bar\gamma\right)$, which completes the proof.
\end{proof}

%\begin{align*}
%\pi\circ(\epsilon_k\otimes\delta)\left(\left(\left(\Phi_B^R\otimes I_p\right)_{ij}\otimes 1\right)\left(1\otimes \left(\Phi_{\bar\gamma}^R\right)_{ij}\right)\right) &= \pi\circ(\epsilon_k\otimes\delta)\left(\left(\left(\Phi_B^L\otimes I_p\right)_{ij}\otimes 1\right)\left(1\otimes \left(\Phi_{\bar\gamma}^L\right)_{ij}\right)\right)\\
%&= \Delta\left(\fp B\bar\gamma\right)
%\end{align*}
...
%$\psi(\phi_{\subpp{\s_n}^{\pm 1}}(a_{ij})) = (\phi_{\s_n^{\pm 1}} \otimes \id)(\psi(a_{ij}))$, $1 \le i,j \le pk$.
\end{lem}
...
Let $X_n^{(p)} = \{(n-1)p+1,\ldots,np\}$, and let $Y = X_{(n-1)p+1,j-np-1}$\todo[color=\ltblue]{define $X_{m,l}$}  We have\todo[color=\ltblue]{change conditions to match first}
$$
...
\begin{figure}[ht]
      \begin{tikzpicture}[scale=.5,>=stealth]
        % background
        \foreach \c in {-6,4,12} \draw (\c,1) circle (60pt);
          \foreach \p in {-1.35,-1.15,-.75,-.55} \filldraw (-6+\p,1) circle (1.5pt);
          \foreach \p in {-1.35,-1.15,-.75,-.55} \filldraw (4+\p,1) circle (1.5pt);
          \foreach \p in {-1.35,-1.15,-.75,-.55} \filldraw (12+\p,1) circle (1.5pt);
        ...
        % spanning arcs
        \draw[->] (-6.55,1) ..controls (-6.8,1.5) and (-7,0.75) ..(-7.25,0.75)
                                  ..controls (-7.6,0.75) and (-7.6,0.9) ..(-7.6,1)
                                  ..controls (-7.6,2.5) and (-4.5,2) ..(-3.9,1);
        \draw[->] (3.45,1)  ..controls (3.3,1.3) and (3,1.3) ..(2.85,1);
        \draw[->] (11.45,1)  ..controls (11.25,1.6) and (10.85,0.4) ..(10.65,1);
        \draw (-1,1) node {\footnotesize $=\quad c_{4,\ast}\quad +$};
        \draw (6.2,1) node[right] {\footnotesize $c_{2,\ast} \quad +$};
        \draw (14.2,1) node[right] {\footnotesize $c_{1,\ast}$};
      \end{tikzpicture}
      \caption{$\pp[2]\tau\cdot c_{2,\ast}$ as an element of $S_{2n}^L$}
      \label{FigTauInSn}
    \end{figure}
...
As discussed below, Theorem \ref{thm:ChainRule} relates the $\Psi_{ij}^k$ for different choices of $k$. %For each $1\le i,j\le n$, each entry of $\Psi_{ij}^k$ is a polynomial in the (non-commuting) variables ${\bf a} = (a_{12},\ldots,a_{1,2p},a_{21},\ldots,a_{2p,2p-1})$.
%For $1\le i,j\le p$ the $(i,j)$ entry of ${\bf X^k}$ is a polynomial, written $X_{ij}^k({\bf a})$, in the (non-commuting) variables ${\bf a} = (a_{12},\ldots,a_{1,2p},a_{21},\ldots,a_{2p,2p-1})$.

%Recalling Section \ref{SecBG_KCHdef}, consider the $(p+1)^{st}$ row of $\Phi_{\gamma\subsmallp(\alpha\subsmallp)}^L$ given by considering the spanning arc $\gamma\smallp(\alpha\smallp)\cdot c_{p+1,\ast}$ which is depicted in Figure \ref{FigPhiLCalcPhi}.

%\begin{lem}Suppose $\delta:\mathcal A_{2n}\to \mathbb C$ is a homomorphism with the property $\delta((\Phi_{\subpp[2]\alpha}^L)_{2i-1})={\bf e}_{i}$ for $2< i\le 2n$. Then $\delta(B_{11}^{n+1}({\bf a})) = 0$.
%\label{LemZeroness}
%\end{lem}
...
By Theorem \ref{thm:ChainRule} we have $\Phi_{\subpp\tau^{k+1}}^L = \phi_{\subpp\tau}(\Phi_{\subpp\tau^{k}}^L)\Phi_{\subpp\tau}^L$. Thus by the above claim, for $1<j\le n$,
  \begin{equation}
    %A_{i,j}^{k+1}({\bf a}) &= A_{i,j-1}^k(\phi_{\subpp[2]\tau}({\bf a})) \\
    \Psi_{ij}^{k+1} = \phi_{\subpp\tau}(\Psi_{i,j-1}^k).
    %C_{i,j}^{k+1}({\bf a}) &= C_{i,j-1}^k(\phi_{\subpp[2]\tau}({\bf a})) \\
    ...
    \begin{figure}[ht]
      \begin{tikzpicture}[scale=.75,>=stealth]
        ...
        \foreach \c in {0,5} \draw (\c,1) circle (60pt);
          \foreach \p in {-1.35,-1.15,-.75,-.55,-.15,.05,1.25,1.45} \filldraw (5+\p,1) circle (1.5pt);
        \foreach \c in {0}
          \foreach \p in {-1,-.6,.6,1} \filldraw (\c+\p,1) circle (1.5pt);
        \foreach \c in {0,5}  
          \draw   (\c+2.1,1) node {{\large $\ast$}}
              (\c+0.6,2.75) node {$D$};
        \foreach \c in {5}
        ...
        \draw[->] (-1,1)  ..controls (-0.5,0.2) and (1.25,1.7) ..(1.25,1)
                          ..controls (1.25,0.3) and (-1.25,0.3) ..(-1.25,1)
                          ..controls (-1.25,1.6) and (1.8,2.25) ..(2.05,1.05);
        %\draw[->] (4,1)   ..controls (4.5,0.2) and (6,1.7) ..(6,1);
        %\draw[->] (4,1)   ..controls (4.5,0.2) and (5.6,1.7) ..(5.6,1);
        \draw (5,-1.25) node[below] {{\small Case $n>2$}};
        \draw (0,-1.25) node[below] {{\small Case $n=2$}};
      \end{tikzpicture}
...
%Per the braid satellite construction denote by $\overline\gamma$ the inclusion $\gamma\in B_p\hookrightarrow B_{2p}$. Upon considering the spanning arc $\overline\gamma\cdot c_{p,\ast} = c_{1,\ast}$ we see that the $p^{th}$ row of $\Phi_{\overline\gamma}^L$ is ${\bf e}_1=(1,0,\ldots,0)$. As $(\Phi_{\gamma\subsmallp(\alpha\subsmallp)}^L)_p = \phi_{\subpp\alpha}(\Phi_{\overline\gamma}^L)\Phi_{\subpp\alpha}^L$ we see that the $p^{th}$ row of $\Phi_{\gamma\subsmallp(\alpha\subsmallp)}^L$ is the first row of $\Phi_{\subpp\alpha}^L$. Thus 
%    \[(\Phi_{\gamma\subsmallp(\alpha\subsmallp)}^L)_{pp} = \delta(A_{1p}^3({\bf a})) = \delta(A_{1p}^1({\bf a})) = -\delta(a_{p+1,p}) = 0.\]
%This gives us a contradiction, proving the statement for $n=2$.
