...
More precisely, suppose that $B\in B_k$ and $B'\in B_p$ are two braids (here $B_n$ denotes Artin's group of $n$-braids) and that $K$ is the braid closure of $B$ (see Figure \ref{fig:BClosure} below). Denote by $\fp B$ the braid in $B_{kp}$ which consists of each strand of $B$ being replaced by $p$ parallel copies of that strand (in the blackboard framing). Also, include $B'$ into $B_{kp}$ by juxtaposing it with an identity braid on $(k-1)p$ strands. The \emph{braid satellite} of $K$ associated to $B, B'$ is the braid closure of $\fp BB'$, denoted $K(B,B')$.
...
where $c_Y = (-1)^{|Y|+1}$ if $Y\cap (W\cup\{\max{W} + 1\}) = \emptyset$, $c_Y = (-1)^{|Y|}$ if $Y\cap W\ne\emptyset$, and $c_Y = 0$ otherwise.\todo{this is perhaps a bad way of putting this, since when $c_Y = 0$, we have the undefined $a_{\max{W}+1,\max{W}+1}$ cord appearing in the sum}
...
Let $X_n^{(p)} = \{(n-1)p+1,\ldots,np\}$, and let $Y = X_{(n-1)p+1,j-np-1}$  We have
$$
\phi_{\fp\s_n}(a_{ij}) =
\left\{
     \begin{array}{lr}
       ...
       B'(i+p,j-p,X_n^{(p)},Y) & \colon (n-1)p< i\le np< j \le (n+1)p\\
...
Let $\a_i  = np-p+r_i$.  Note that if $np-p+1\le y_1<\a_i$ then $\psi(a_{iy_1}) = 0$, and if $np \ge y_k>\a_j$ then $\psi(a_{y_kj}) = 0$, so the sum on the right hand side can be taken over $Y\subseteq\{\a_i,\a_i+1,\ldots,\a_j\}$.  Then we manipulate the sum to get\todo{do I need to explain what I'm doing here?}
...
$$\psi\left(\phi_{\fp \s_n}(a_{ij})\right) = \sum_{Y\subseteq \{np-p+1,\ldots,np\}}(-1)^{|Y|}\psi\left(a_{i + p,y_k}a_{y_{k-1}y_{k-2}}\cdots a_{y_1,j}\right)$$
%&= \psi\left(A(i,j+p,\{np-p+1,\ldots,np\})\right)\\
...
%\sum_{Y\subseteq \{np-p+1,\ldots,np\}}(-1)^{|Y|}\psi\left(a_{i + p,y_k}a_{y_{k-1}y_{k-2}}\cdots a_{y_1,j}\right)$$
%&= \psi\left(A(i,j+p,\{np-p+1,\ldots,np\})\right)\\
