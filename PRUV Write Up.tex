...
(\epsilon_k\otimes\epsilon_p)\psi\left(\phi_{B^{(p)}}\left(\Phi_{B'}^L\right)\right)\psi\left(\Phi_{B^{(p)}}^L\right)
    ...
    & = \operatorname{diag}((-1)^{w(B)},1,\ldots,1)\otimes\operatorname{diag}((-1)^{w(B')},1,\ldots,1).
\end{align*}

\noindent But $w(B)$ is even, which also implies that $w(B^{(p)})$ is even, so the right hand side is equal to $\operatorname{diag}((-1)^{w(B^{(p)}B')},1,\ldots,1)$, as desired.
