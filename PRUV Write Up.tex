...
Let $K$ be a knot in $S^3$, and let $B\in B_n$ be a braid closing to $K$.  Throughout this paper we will use the framing normal to $B$. \todo{not sure how to say this}  We denote by $\pi_K$ the fundamental group of knot complement $\overline{S^3\setminus n(K)}$.  An element of $\pi_K$ is a \emph{meridian of $K$} if it can be represented by a disc $D$ embedded in $\R^3$ such that $D$ intersects $K$ exactly once on the interior of $D$.  The \emph{meridional rank} of $K$, written $\mr(K)$, is the minimal size of a meridional generating set of $\pi_K$.  The \emph{bridge number} of $K$, denoted $b(K)$, is the minimum number of local maxima of $K$ taken over all embeddings of $K$ into $\R^4$ with a height function.\todo{is there a better way to say this?}
...
Fix $p>0$ and let $B$ be a braid on $k$ strands. For each $1\le i\le pk$ define integers $q_i,r_i$ such that $i = q_ip + r_i$, where $0<r_i \le p$. Instrumental to the proof of Theorem \ref{main} will be the map $\psi\colon \A_{pk}\otimes R_0 \rightarrow \left(\A_k\otimes R_0\right)\otimes \left(\A_p\otimes R_0\right)$, defined as follows
...
\noindent We also define $\psi(\mu) = \mu\otimes 1$ and $\psi(\lambda) = \lambda\otimes 1$.  Note that $\psi(a_{ij}) \in 1\otimes \A_p$ or $\psi(a_{ij}) = 0$ if and only if $q_i = q_j$, and that $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$.  This homomorphism gives us a way of relating $\Phi_{B^{(p)}}^L$ to $\Phi_B^L$ via the following proposition.
