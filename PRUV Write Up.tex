...
Furthermore, $\phi_\beta(\overline{a_{ij}}) = \overline{\phi_\beta(a_{ij})}$ for any braid $\beta$ \todo{does this need justification?} and $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$, so it suffices to prove the lemma for $\pp{\s_n}$ in the case where $i<j$.
