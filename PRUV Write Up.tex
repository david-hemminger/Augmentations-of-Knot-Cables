...
For each $1\le i\le pk$ let $q_i\ge 0$ and $0<r_i\le p$ be the integers such that $i = q_ip + r_i$. We define the map $\psi\colon \A_{pk} \rightarrow\A_k\otimes\A_p$ on generators as follows
...
\noindent Note that $\psi(a_{ij}) \in 1\otimes \A_p$ and is nonzero if and only if $q_i = q_j$, and that $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$.  This homomorphism gives us a way of relating $\Phi_{\fp B}^L$ to $\Phi_B^L$ via the following proposition.
...
By Theorem \ref{thm:RanknAugs} it suffices to prove that there exists a homomorphism $\epsilon\colon \A_{pk}\rightarrow \C$ such that $\epsilon\left(\Phi_{\fp BB'}^L\right) = \epsilon\left(\Phi_{\fp BB'}^R\right) = \Delta(\fp BB')$. Additionally, our assumptions imply that there exist augmentations $\epsilon_k\colon \A_k\otimes R_0 \rightarrow \C$ and $\epsilon_p\colon \A_p\otimes R_0 \rightarrow \C$ such that $\epsilon_k\left(\Phi_B^L\right) = \epsilon_k\left(\Phi_B^R\right) = \Delta(B)$ and $\epsilon_p\left(\Phi_{B'}^L\right) = \epsilon_p\left(\Phi_{B'}^R\right) = \Delta(B')$.

We will define a homomorphism $\delta\colon\A_p\rightarrow \C$ such that $\delta = \pm\epsilon_p$, the sign depending on the parity of $w(B)$ and $p$. If $\pi\colon \C\otimes \C \rightarrow \C$ is the homomorphism determined by $\pi(a\otimes b) = ab$, for $a,b\in\C$, then we set $\epsilon = \pi\circ(\epsilon_k\otimes\delta)\circ\psi$.  %We will later break the theorem up into three cases depending on the parity of $w(B)$ and $p$ and in each case define $\delta$ so that $\epsilon$ is an augmentation of $\fp BB'$. 
...
Note that since the non zero or one entries of $\Phi_{B'}^L$ are products of $a_{ij}$ where $i<j \le p$, $\phi_{\fp B}$ takes each of the $a_{ij}$'s in these products to $a_{i + mp, j+mp}$ for some $0\le m<k$ (this can be seen easily by using the alternate description of $\phi_{\fp B}$ detailed in Section \ref{SecBG_KCHdef} and using that $\fp B$ is a product of braids $\fp\s_l$).  We have that $\psi$ takes $a_{i + mp, j+mp}$ to $1\otimes a_{ij}$, however, so

$$\psi\left(\phi_{\fp B}\left(\Phi_{B'}^L\right)\right) = \left(1\otimes \left(\Phi_{B'}^L\right)_{ij}\right)$$

\noindent By Proposition \ref{psiofbp}

$$\psi\left(\Phi_{\fp B}^L\right) = \left(\left(\Phi_B^L\right)_{ij}\otimes 1\right)\otimes I_p = \left(\left(\Phi_B^L\otimes I_p\right)_{ij}\otimes 1\right).$$
...
\begin{align*}
\psi\left(\Phi_{\fp B}^L\right) &= \psi\left(\phi_{B'^{(p)}}\left(\Phi_{\fp\s^{q_r}_{n_r}}^L\right)\cdot\Phi_{B'^{(p)}}^L\right)\\
