...
\end{restatable}
  ...
  \begin{restatable}[\cite{Cor13a}]{theorem}{thmAugKCH_Corresp}
  ...
  \begin{restatable}[\cite{Cor13b}]{theorem}{thmDimBound} Given a knot $K\subset S^3$, if $g_1,\ldots,g_d$ are meridians that generate $\pi_K$ and $\rho:\pi_K\to\text{GL}_r\C$ is a KCH irrep then $r\le d$.
  \label{thm:DimBound}
  \end{restatable}
...
\begin{restatable}[\cite{Cor13b}]{theorem}{corDimBound}
\label{cor:DimBound}
$\text{ar}(K)\le\text{mr}(K)\le b(K)$
\end{restatable}    
...
\section{Main Result}
\label{SecMain}
\todo[color=\ltblue,inline]{fix augmentations to include $R_0$}
