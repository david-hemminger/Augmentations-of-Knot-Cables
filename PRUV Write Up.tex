...
A basic argument shows that $\mr(K)\le b(K)$ for any $K\subset S^3$. Whether the bound is equality for all knots is an open question attibuted to Cappell and Shaneson \cite[Prob. 1.11]{Kir95}. Equality is known to hold for some families of knots due to work of various authors. 

Here we study the behavior of \emph{augmentations}, which are maps that arise in the study of knot contact homology. To each augmentation is associated a rank, and there is a maximal rank of augmentations of a given $K$, called the \emph{augmentation rank} $\ar(K)$. For any $K$ the inequality $\ar(K)\le \mr(K)$ holds (see Section \ref{SecBG_AugRk} below). 

Our main result indicates how $\ar(K)$ behaves under satellite operations with a braid pattern, provided the companion and pattern have sufficiently large augmentation rank. To be precise let $B\in B_k$, and $B'\in B_p$, be braids on $k$, and $p$, strands respectively. Write $\widehat{B}$ for the \emph{braid closure} of $B$ (see Section \ref{SecBG}, Figure \ref{fig:BClosure}) and set $K = \widehat{B}$. Throughout the paper only braid closures that are knots are considered.

\begin{defn} Denote by $\fp B$ the braid in $B_{kp}$ which consists of each strand of $B$ being replaced by $p$ parallel copies of that strand (in the blackboard framing). Upon including $B'$ into $B_{kp}$ by juxtaposing it with an identity braid on $(k-1)p$ strands, define the \emph{braid satellite} of $K$ associated to $B, B'$ to be $K(B,B') = \widehat{\fp B\cdot B'}$.
\label{defn:BraidSat}
\end{defn}
\todo[color=\ltgreen]{make a figure} 
\todo[color=\ltgreen]{remark on difference from $B'(K)$ in satellite literature}

When $\widehat{B}$ and $\widehat{B'}$ are each a knot, $K(B,B')$ is also. We show the following. 

\begin{thm}\label{main}
If $B\in B_k$ is such that $\ar(\widehat{B})=k$ and $B'\in B_p$ such that $\ar(\widehat{B'})=p$, then $\ar(K(B,B'))=kp$.
\end{thm}

\begin{rem}%Our theorem shows that if $B, B'$ have largest possible augmentation rank (equal to their index) then the augmentation rank acts multiplicatively under braid satellites. 
It is natural to ask if the augmentation rank is multiplicative under weaker assumptions on $B, B'$. Braid stabilization can show it is submultiplicative at best. In Section \ref{SecComments} we show that it is not submultiplicative in general.
\label{rem:BrSatNotMult}
\end{rem}

We obtain a corollary involving Cappell and Shaneson's question for iterated torus knots. Let ${\bf p}=(p_1,\ldots,p_n)$ and ${\bf q}=(q_1,\ldots,q_n)$ be integral vectors. We write $T({\bf p},{\bf q})$ for the $({\bf p},{\bf q})$ \emph{iterated torus knot}, defined as follows. Define $T({\bf p},{\bf q})$ inductively so that, if $\widehat{\bf p}, \widehat{\bf q}$ are the truncated lists obtained from ${\bf p},{\bf q}$ by removing the last integer in each, then $T({\bf p},{\bf q})$ is the $(p_n,q_n)$-cable of $T(\widehat{\bf p},\widehat{\bf q})$. By convention $T(\emptyset,\emptyset)$ is the unknot.

The knot $T({\bf p},{\bf q})$ is well-defined only if a framing convention is chosen at each stage of cabling. In contrast to the traditional choice of Seifert framing at each stage, we choose a framing so that if $B$ is of minimal braid index such that $T(\widehat{\bf p},\widehat{\bf q}) = \widehat{B}$ then $T({\bf p},{\bf q}) = K(B,(\s_1\ldots\s_{p_n-1})^{q_n})$.
...
The paper is organized as follows. In Section \ref{SecBG} we give the needed background in knot contact homology and discuss the rank of augmentations and the relationship to the meridional rank. In Section \ref{SecBG_AugExist} we review some techniques that will be used in the proof of Theorem \ref{main}. Section \ref{SecMain} is mainly devoted to the proof of Theorem \ref{main}, and in Section \ref{SecComments} we consider the more general case.
...
\label{SecBG}
  ...
  Throughout the paper we denote by $B_n$ the group of $n$-braids. We orient $n$-braids from left to right, labeling the strands $1,\ldots, n$, with 1 the topmost and $n$ the bottommost strand. We work with Artin's generators $\{\sigma_i^{\pm}$, $i=1,\ldots,n-1\}$ of $B_n$, where in $\s_i$ only the $i$ and $i+1$ strands interact, and they cross once in the manner depicted in Figure \ref{fig:BraidGens}.
      \begin{figure}[ht]
        ...
        \label{fig:BraidGens}
      \end{figure}
    Given a braid $B$ the braid closure $\widehat{B}$ of $B$ is the link obtained as shown in Figure \ref{fig:BClosure}. The \emph{writhe} (or algebraic length) of $B$, denoted $\omega(B)$, is the sum of exponents of the Artin generators in a word representing $B$.
...
In this section we address two questions arising from Theorem \ref{main}. We first consider, in Section \ref{SecMultiplic}, the possibility of weaker assumptions for which the multiplicative property of the theorem still holds - that is, what is necessary for $\ar(K(B,B')) = \ar(\widehat{B})\ar(\widehat{B'})$? We will provide some examples showing that augmentation rank is not multiplicative (nor sub-multiplicative) in the general case. 

Second, we consider in Section \ref{SecDehOrder} how atypical is the equality  $\ar(B)=n$ (for $B\in B_n$). The comments of this section are somewhat speculative. We recall the Dehornoy order on the braid group and remark on the possibility of a relationship to the augmentation rank.

\subsection{Augmentation rank does not multiply}
\label{SecMultiplic}

The knot $K(B,B')$ obtained by the braid satellite construction depends on the braid $B$ chosen to represent $\widehat{B}$ (as we are using $B$ to determine the framing). It is thus not surprising that stabilization can be used to find $B, B'$ such that $\ar(K(B,B')) < \ar(B)\ar(B')$. 

In fact, for $B\in B_k$ define $S^{\pm}(B)$ as the stabilization $\iota(B)\s_1^{\pm}\in B_{k+1}$ where $\iota:B_k\to B_{k+1}$ is the map given on generators by $\iota(\s_i) = \s_{i+1}$ for $1\le i\le k-1$. Then for any $B'\in B_p$, a knot equal to $K(S^{\pm}(B),B')$ is also equal to the braid satellite $K(B,\Delta^{\pm 2}B')$, where $\Delta^2$ is the full twist in $B_p$. This can be seen by ``destabilizing'' the companion torus in the satellite (see Figure \ref{}). 

Setting $k = 1$ and taking $B'$ so that $\ar(\widehat{B'}) = p$ and $\ar(\widehat{\Delta^{\pm}B'})<p$ gives that $\ar(K(S^{\pm}(B),B')) = \ar(\widehat{\Delta^{\pm}B'}) < p = \ar(\widehat{B})\ar(\widehat{B'})$. As a less trivial example, we remark that (taking $\s_1\in B_2$) we have $\ar(K(\s_1^3,\s_1)) < 4$, however $K(\s_1^3,\s_1) = K(S^{-1}(\s_1^3),\s_1^3)$ and $\ar(\widehat{S^{-1}(\s_1^3)}) = \ar(\widehat{\s_1^3}) = 2$. (Note, in more conventional notation, $K(\s_1^3,\s_1)$ is the $(2,7)$-cable of the right-handed trefoil.

It should be observed that every example thus far where the rank is submultiplicative (strictly) has involved taking a $B$ which is not minimal braid index. It can occur that the rank is super-multiplicative. By finding a solution to (\ref{eqn:FindingAugs}) for $B = {\scriptstyle \bar{\bar{2}}}\s_1^5\cdot \s_1\in B_4$ we can show that $\ar(K(\s_1^5,\s_1)) = 4$, even though $\ar(\widehat{\s_1^5})=2$ and $\ar(\widehat{\s_1})=1$.
\todo[color=\ltgreen]{insert observations on exchange moves. conjecture should be that supermultiplicative holds for minimal braids?, maybe even multiplicative if unique conjugacy class in closure?}

\subsection{Braid closures with full rank}
\label{SecDehOrder}

We recall the Dehornoy order on $B_n$, which is a total, right invariant order $<_D$ on the braid group. A braid $B\in B_n$ is \emph{positive} in $B_n$ if $e <_D B$ and \emph{negative} if $B <_D e$, where $e$ denotes the identity element of $B_n$. Right invariance of the ordering immediately implies that $B$ is positive if and only if $B^{-1}$ is negative.

Let $\Delta = (\s_1\ldots\s_{n-1})(\s_1\ldots\s_{n-2})\ldots(\s_1)$ be the half twist in $B_n$. Recall that $\Delta^2=(\s_1\ldots\s_{n-1})^n$ generates the center of $B_n$ \cite{}. As a consequence, if $\Delta^2 <_D B$ then $\Delta^{-2}B$ is positive which implies that $\Delta^{2m} <_D B^m$ for any $m>0$.

Early calculations seem to indicate that the lack of a solution to equation (\ref{eqn:FindingAugs}) can only occur for braids that are in a sufficiently small interval about $e$ in the Dehornoy order. It is a remarkable fact, resulting from Birman and Menasco's Markov Theorem without Stabilization \cite{BM_MTWS}, that for each braid index $n$ there is a number $m_n$ such that if $\Delta^{2m_n} <_D B$ in $B_n$ then $n$ is the minimal braid index of the closure of $B$, and every braid in $B_n$ with the same closure as $B$ is conjugate to $B$. By results in \cite{} it must be that $m_n \ge n-1$.

Note that if $\Delta^{2m_n} <_D B$ then $\Delta^{-2m_n}B$ is positive and so $B = \Delta^{2m_n}\beta$, where $\beta$ is positive.

If the closure of $B\in B_n$ is $K$ and $b(K)<n$ then there cannot be rank $n$ augmentations. For each 2-bridge knot $K$, up to crossing number 10, which has minimal braid index 3 we have taken a braid $B$ which has closure $K$ and calculated that $\ar(\Delta^2B)=3$ or $\ar(\Delta^4B)$ (here $\Delta$ is the half twist in $B_3$. In addition, it appears that if $\ar(\Delta^{2l}B)=n$ then $\ar(\Delta^{2m}B)=n$ for $m>l$. Such calculations can sometimes be done for braids in $B_4$, though they can quickly become prohibitively complicated as braid length increases.

We remark that it is possible to find strictly increasing sequences of braids (which remain less than a fixed power of $\Delta^2$) where the augmentation rank remains constant, and less than $n$. 

{\bf Question:} For a given braid index $n$, does there exist a number $m_n$ such that if $\Delta^{2m_n} <_D B$ then $\ar(B) = n$?
