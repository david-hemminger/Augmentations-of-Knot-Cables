...
Fix $\alpha\in B_{kp}$ and $1\le i\le kp$.  By the definitions of $\Phi_{\alpha}^L$ and $\Phi_{\subpp{\alpha}}^L$ and Lemma \ref{commutes} we have that

\begin{align*}
\left(\sum_{l=1}^k \left(\Phi_\alpha^L\right)_{q_i+1,l}a_{l*}\right)\otimes a_{r_i*} &= \left(\phi_{\alpha}\otimes \id\right)\Psi\left(a_{i*}\right)\\
&= \Psi\left(\phi_{\subpp{\alpha}}\left(a_{i*}\right)\right)\\
&= \sum_{j=1}^{kp} \Psi\left(\left(\Phi_{\subpp{\alpha}}^L\right)_{ij}\right)\left(a_{q_j + 1,*}\otimes a_{r_j*}\right)\\
\end{align*}

\noindent Thus we have that $\Psi\left(\left(\Phi_{\subpp{\alpha}}^L\right)_{ij}\right) = 0$ if $r_i \ne r_j$ and $\Psi\left(\left(\Phi_{\subpp{\alpha}}^L\right)_{ij}\right) = \left(\Phi_\alpha^L\right)_{q_i+1,q_j+1}$ if $r_i = r_j$, so $\Psi\left(\Phi_{\subpp{\alpha}}^L\right) = \Phi_\alpha^L\otimes I_p$.  Since $\Phi_\alpha^R = \overline{\Phi_\alpha^L}^t$ and $\Psi(\overline{a_{ij}}) = \overline{\Psi(a_{ij})}$, we then have that $\Psi\left(\Phi_{\subpp{\alpha}}^R\right) = \Phi_\alpha^R\otimes I_p$ as well.
\end{proof}
...
%%Proof of psiofbp
%\begin{proof} [Proof of Proposition \ref{psiofbp}]
%Let $\alpha = \s_{n_1}^{q_1}\cdots\s_{n_r}^{q_r}$, where $1\le n_i<k$ and $q_i = \pm 1$.  We will prove the proposition by induction on $r$.  The base case is already taken care of by Lemma \ref{basecase}.  Suppose that the proposition holds for braids of length $r-1$.  Let $\tilde{\alpha} =\s_{n_1}^{q_1}\cdots\s_{n_{r-1}}^{q_{r-1}}$ Then by the Chain Rule and Lemmas \ref{commutes} and \ref{basecase}, we have that
%\begin{align*}
%\psi\left(\Phi_{\subpp\alpha}^L\right) &= \psi\left(\phi_{\subpp{\tilde{\alpha}}}\left(\Phi_{\subpp{\s_{n_r}}^{q_r}}^L\right)\cdot\Phi_{\subpp{\tilde{\alpha}}}^L\right)\\
%&= \left(\phi_{\tilde{\alpha}}\otimes\id\right)\left(\psi\left(\Phi_{\subpp{\s_{n_r}}^{q_r}}^L\right)\right)\cdot \left(\Phi_{\tilde{\alpha}}^L\otimes I_p\right)\\
%&= \left(\phi_{\tilde{\alpha}}\otimes\id\right)\left(\Phi_{\s_{n_r}^{q_r}}^L\otimes I_p\right)\cdot \left(\Phi_{\tilde{\alpha}}^L\otimes I_p\right)\\
