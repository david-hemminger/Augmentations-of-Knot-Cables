...
We begin in Section \ref{MainProof} with an intermediate result, Proposition \ref{psiofbp}, followed by the proofs of our main theorem and Corollary \ref{cor:iteratedCables}.  In Section \ref{PropAndLemmas} we prove Lemma \ref{commutes}, which is used to prove Proposition \ref{psiofbp}, as well as Lemma \ref{Sigma_n}, which is used to prove Lemma \ref{commutes}.
...
\noindent which determines an algebra map $\psi\colon \A_{pk} \rightarrow \A_k\otimes \A_p$. Also, if we extend conjugation to $\A_k\otimes\A_p$ by applying it to each factor, then $\psi(\overline{a_{ij}}) = \overline{\psi(a_{ij})}$. We have the following proposition.
...
Suppose that ${\bf p}$ and ${\bf q}$ have $n$ entries and $\ar(T({\bf \hat p},{\bf \hat q})) = p_1p_2\cdots p_{n-1}$.  Let $\alpha = T({\bf \hat p},{\bf \hat q})$, and let $\gamma = (\sigma_1\ldots \sigma_{p_n-1})^{q_n}$.  Then Theorem 1.3 from \cite{Cor13b} gives that $\ar(\gamma) = p_n$, and since $T({\bf \hat p},{\bf \hat q}) = K(\alpha,\gamma)$, Theorem \ref{main} gives the desired result.
\end{proof}
