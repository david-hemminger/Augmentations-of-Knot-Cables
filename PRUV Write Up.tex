...
Our main result indicates how $\ar(K)$ behaves under satellite operations with a braid pattern, provided the companion and pattern have sufficiently large augmentation rank. To be precise let $B\in B_k$, and $B'\in B_p$, be braids on $k$, and $p$, strands respectively. Write $\widehat{B}$ for the \emph{braid closure} of $B$ (see Section \ref{SecBG}, Figure \ref{fig:BClosure}) and set $K = \widehat{B}$. 
...
By Theorem \ref{main} if $B\in B_k$ and $B'\in B_p$ have closures with augmentation ranks $k$ and $p$, respectively, then we have the multiplicative property $\ar(K(B,B')) = \ar(B)\ar(B') = kp$. We are lead to ask two questions: first, whether the multiplicative property holds under some weaker assumption; second, how atypical the property $\ar(B)=n$ (for $B\in B_n$) is. Section \ref{SecMultiplic} addresses the first point. We show that the augmentation rank is not even submultiplicative in the most general case. Section \ref{SecDehOrder} is somewhat speculative. We recall the Dehornoy order on the braid group. It might be that the closures of sufficiently large braids in this order have augmentation rank equal to their braid index.

\subsection{Augmentation rank does not multiply}
\label{SecMultiplic}

It is not difficult to find $B, B'$ such that $\ar(K(B,B')) < \ar(B)\ar(B')$. Letting $K$ (resp.\ $K'$) denote the closure of $B$ (resp.\ $B'$), recall (\ref{Eqn:ARvMRvb}) which gives the string of inequalities
        \begin{equation}
          \ar(K)\le\mr(K)\le b(K) \le n
        \end{equation}
for any knot which is the closure of a braid in $B_n$. Hence, if $b(K) = \ar(K)$, $b(K') = \ar(K')$, and $b(K(B,B')) < b(K)b(K')$ then it must be that $\ar(K(B,B')<\ar(B)\ar(B')$. By a result of Schubert \cite{}, this can only occur if $K$ is the unknot. For example let $B = \s_1^{-1}\in B_2$ and $B' = \s_1^3\in B_2$. Then $\ar(K) = 1 = b(K)$, $\ar(K') = 2 = b(K')$. However, as $K(B,B')$ is the unknot, $\ar(K(B,B')) = 1$. 

It can also occur that the rank is submultiplicative when $b(K(B,B'))=b(K)b(K')$. An example is shown in Figure \ref{fig21cable}. By calculation one finds that $\ar(K(\s_1^3,\s_1)) < 4$. However, Schubert's result says that $b(K(\s_1^3,\s_1)) = 4$. A slight modification of this example shows that augmentation rank is not even submultiplicative, for $\ar(K(\s_1^5,\s_1)) = 4$ even though $\ar(\widehat{\s_1^5})=2$ and $\ar(\widehat{\s_1})=1$.
