...
Fix $p>0$ and let $B$ be a braid on $k$ strands. For each $1\le i\le pk$ define integers $q_i,r_i$ such that $i = q_ip + r_i$, where $0<r_i \le p$. Instrumental to the proof of Theorem \ref{main} will be the map $\psi\colon \A_{pk}^{ab} \rightarrow \A_k^{ab}\otimes \A_p^{ab}$, defined as follows (note that since \todo[color=\ltblue]{extend definition to when $i>j$} $a_{ij}\in \A_{pk}^{ab}$, $i<j$, so we must have $q_i\le q_j$):
